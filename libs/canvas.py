...
# from PyQt5.QtGui import QCursor, Qt, QColor
...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        """Handle mouse movement events, including drawing, moving shapes, and highlighting."""
        try:
            pos = self.transform_pos(ev.pos())  # Transform the mouse position based on current scaling/translation

            # Update coordinates in the status bar if an image is opened
            ...
            if self.drawing():
                ...
                if self.current:
                    ...
                    if self.out_of_pixmap(pos):
                        # Don't allow the user to draw outside the pixmap, clip the position to valid bounds
                        size = self.pixmap.size()
                        clipped_x = min(max(0, pos.x()), size.width())
                        clipped_y = min(max(0, pos.y()), size.height())
                        pos = QPointF(clipped_x, clipped_y)
                    elif len(self.current) > 1 and self.close_enough(pos, self.current[0]):
                        # Attract line to starting point and alert user
                        pos = self.current[0]
            ...
            # Polygon copy moving (Right-click)
            ...
            # Polygon/Vertex moving (Left-click)
            ...
            # Hovering over the canvas: Check if we're near any shape or vertex.
            ...
            for shape in reversed([s for s in priority_list if self.isVisible(s)]):
                # Look for a nearby vertex to highlight. If that fails, check if inside a shape.
                ...
                if index is not None:
                    ...
                    # Display annotation width and height while hovering inside the shape
                    point1 = self.h_shape[1]
                    point3 = self.h_shape[3]
                    current_width = abs(point1.x() - point3.x())
                    current_height = abs(point1.y() - point3.y())
